...
/**
 *  sina qq 、豆瓣 local 输出的 profile 格式不一致，进行格式化
 *
 * @param provider 认证提供商
 * @param profile 用户信息
 */
function formatProfile(provider, profile) {
  var user = {};
  user.provider = provider;
  switch (provider) {
    case 'qq':
      //myProvide = _.pick(profile,['provide','id']);
      user.username = profile.nickname;
      user.description = profile._json.msg;
      var info = _.pick(profile._json, ['gender', 'province', 'city', 'year', 'figureurl', 'figureurl_1', 'figureurl_2']);
    ...
    case 'douban':
      ...
      user.website = profile.profileUrl;
      user.figureurl = profile.avatar;
      user.location = profile._json.loc_name;
      user.description = profile._json.signature;
      break;
...
passport.connect = function (req, query, profile, next) {
  ...
  // 用户数据格式化
  user = formatProfile(provider, profile);
